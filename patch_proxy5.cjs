...
const search = `async function handleStreamProxy(req: Request, res: Response, type: 'live' | 'movie' | 'series') {
  try {
...
const replace = `async function handleStreamProxy(req: Request, res: Response, type: 'live' | 'movie' | 'series') {
  try {
    const { username, password, streamId, ext = type === 'live' ? 'ts' : 'mp4' } = req.params;
